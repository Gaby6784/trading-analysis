...
def get_cache_key(ticker: str) -> str:
    ...
    Returns:
        Cache key string (ticker + hour for 1-hour cache granularity)
    """
    import hashlib
...
def get_cached_news(ticker: str) -> Optional[List[Tuple[str, datetime]]]:
    ...
    Returns:
        List of (headline, datetime) tuples or None if cache miss/expired
    """
...
def cache_news(ticker: str, articles: List[Tuple[str, datetime]]) -> None:
    ...
    Args:
        ticker: Stock ticker symbol
        articles: List of (headline, datetime) tuples to cache
    """
    cache_key = get_cache_key(ticker)
    cache_file = os.path.join(CACHE_DIR, f"{cache_key}.json")
...
def deduplicate_articles(articles: List[Tuple[str, datetime]]) -> List[Tuple[str, datetime]]:
    ...
    unique_articles = []
    for headline, date in articles:
        # Check if this headline is too similar to any existing one
        is_duplicate = False
        for existing_headline, _ in unique_articles:
            if similarity(headline, existing_headline) > 0.85:  # 85% similarity threshold
                is_duplicate = True
                break
...
def fetch_news_headlines(ticker: str, hours: int = 24) -> Tuple[List[Tuple[str, datetime]], str, datetime, datetime]:
    ...
    for source in NEWS_SOURCES:
        try:
            ...
            # If we got articles, cache them and return with source name and date range
            if articles:
...
def analyze_sentiment_simple(headlines: List[Tuple[str, datetime]]) -> float:
    ...
    for headline, pub_date in headlines:
        ...
        # Always ensure timezone-naive for safe datetime arithmetic
...
def generate_recommendation(technicals: Dict, sentiment: float, ticker: str) -> Tuple[str, str]:
    ...
    # STRONG BUY - oversold with trend confirmation
    ...
    # STRONG SELL - overbought with trend confirmation
...
def analyze_ticker(ticker: str) -> Dict:
    ...
    if earnings_date:
        ...
        earnings_date_naive = earnings_date.replace(tzinfo=None) if hasattr(earnings_date, 'tzinfo') and earnings_date.tzinfo else earnings_date
        now_et_naive = now_et.replace(tzinfo=None)
        days_until_earnings = (earnings_date_naive - now_et_naive).days
